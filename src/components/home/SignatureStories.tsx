 ...
 *
 * While the user scrolls vertically through a tall container, the inner
 * content translates horizontally across the viewport. Gives a cinematic
 * "scroll-driven slideshow" experience on desktop and tablet.
 *
 * On reduced-motion or mobile, degrades to a normal vertical stack so
 * nothing is clipped or missed.
 */
const stories = [
  {
...
export default function SignatureStories() {
  ...
  // Total horizontal distance = (cards - 1) / cards → 66.66% shift for 3 cards.
  const shift = useTransform(scrollYProgress, [0.05, 0.95], ['0%', '-66.666%']);
  ...
  return (
    <section
      ...
      // 300vh gives the horizontal pan a comfortable scroll distance.
      className="relative hidden md:block h-[300vh] bg-[#0a0a0a]"
    >
      {/* Sticky viewport — horizontal content slides within */}
      <div className="sticky top-0 h-screen overflow-hidden">
        {/* Ambient gold glow that breathes as user scrolls through */}
        <motion.div
        ...
        {/* Section header — fades in on entry */}
        <motion.div
        ...
        {/* Horizontal track */}
        <motion.div
          className="flex h-full items-center gap-8 lg:gap-14 px-[8vw] pt-40 pb-16"
          style={{
        ...
        >
          {stories.map((story, idx) => (
            <article
              key={story.title}
              className="relative flex-shrink-0 w-[calc(33.333%-3rem)] h-[72vh] rounded-2xl overflow-hidden group"
              style={{
                boxShadow:
                  '0 40px 120px -40px rgba(0,0,0,0.9), 0 0 0 1px rgba(255,255,255,0.04), inset 0 0 0 1px rgba(212,175,55,0.08)',
              }}
            >
              {/* Background image with slow parallax-style scale */}
              <motion.div
              ...
              >
                ...
                <div
                  ...
                  style={{
                    background: `radial-gradient(ellipse at center, ${story.accent}15 0%, transparent 65%)`,
                  }}
                />
              </motion.div>
              ...
              <div className="relative z-10 h-full flex flex-col justify-end p-8 lg:p-10">
                <p
                  className="text-[11px] lg:text-xs tracking-[0.28em] uppercase font-light mb-4"
                ...
                <h3
                  ...
                  style={{ fontSize: 'clamp(1.75rem, 1rem + 1.5vw, 2.5rem)' }}
                >
                  {story.title}
                </h3>
                <div className="w-10 h-px bg-gold/60 mb-4" />
                <p className="text-white/70 text-sm lg:text-base leading-relaxed max-w-md mb-6">
                  {story.description}
                </p>
                <Link
                  href={story.cta.href}
                  className="inline-flex items-center gap-2 text-gold text-sm tracking-[0.18em] uppercase font-medium hover:gap-3 transition-all duration-300"
                >
                  {story.cta.label}
              ...
              <div className="absolute top-6 right-6 z-10">
                <span
                  className="font-playfair italic text-white/20"
                  style={{ fontSize: 'clamp(1.5rem, 1rem + 1vw, 2.25rem)' }}
                >
                  0{idx + 1}
                </span>
              </div>
      ...
      <div className="relative z-10 flex flex-col gap-6 px-4">
        {stories.map((story, idx) => (
          <motion.article
            ...
            className="relative rounded-2xl overflow-hidden h-[60vh] min-h-[420px]"
            style={{
